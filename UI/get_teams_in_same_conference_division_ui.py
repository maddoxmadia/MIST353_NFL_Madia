    ...
    if st.button("Fetch Teams"):
        if not team.strip():
            ...
            if df is not None and not df.empty:
                st.subheader(f"Teams in Same Conference and Division as {team}:")
                st.dataframe(df, use_container_width=True, hide_index=True)
            else:
                st.info(f"No teams found in the same conference and division as {team}. Please check the team name and try again.")